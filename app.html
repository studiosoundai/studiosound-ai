...
#fc-chart{width:100% !important;height:auto !important;}
#fc-reports span{max-width:100%;overflow:hidden;text-overflow:ellipsis;}
.dual-results.show{grid-template-columns:1fr;}
}
</style>
        ...
        <div class="gen-panel">
          ...
          <div class="form-group">
            ...
            <div style="border:1px dashed var(--border);border-radius:8px;padding:20px;text-align:center;cursor:pointer;background:#0f0f1a;" onclick="document.getElementById('audio-upload').click()">
              ...
              <input type="file" id="audio-upload" accept="audio/*,video/mp4,.mp3,.m4a,.wav,.aac,.ogg,.flac,.aif,.aiff" style="display:none;" onchange="convertAudio(this)"/>
            </div>
          </div>
        ...
        <div class="gen-panel">
          ...
          <div class="form-group">
            ...
            <div style="border:1px dashed var(--border);border-radius:8px;padding:20px;text-align:center;cursor:pointer;background:#0f0f1a;" onclick="document.getElementById('trim-upload').click()">
              ...
              <input type="file" id="trim-upload" accept="audio/*,video/mp4,.mp3,.m4a,.wav,.aac,.ogg,.flac,.aif,.aiff" style="display:none;" onchange="loadTrimAudio(this)"/>
            </div>
          </div>
    ...
    <div id="tool-forecast" class="hidden">
      <div class="generator" style="align-items:start;">
        <div class="gen-panel">
          ...
          <div class="form-group">
            <label class="form-label">Upload Royalty Report (CSV / Excel)</label>
            <div style="border:1px dashed var(--border);border-radius:8px;padding:16px;text-align:center;cursor:pointer;background:#0f0f1a;" onclick="document.getElementById('fc-upload').click()">
              ...
              <input type="file" id="fc-upload" accept=".csv,.tsv,.txt,.xls,.xlsx,text/csv" multiple style="display:none;" onchange="fcParseCsv(this)"/>
            </div>
          </div>
          <div style="background:rgba(0,255,209,0.06);border:1px solid rgba(0,255,209,0.3);border-left:3px solid var(--neon);border-radius:8px;padding:8px 12px;margin-top:8px;font-size:12px;color:#e8e8f5;">💡 <strong style="color:var(--neon);">Best practice:</strong> <strong style="color:var(--neon);">upload the exact file your distributor gave you</strong> — converted CSVs often round the cents away, and in streaming the cents are the money. Or type your monthly statement totals below.</div>
          <div id="fc-reports" style="display:flex;flex-wrap:wrap;gap:6px;margin:8px 0 4px;"></div>
          ...
          <div class="form-group">
            ...
            <div style="display:flex;gap:8px;margin-top:8px;">
              <button class="result-btn" onclick="fcAddRow()"><i class="ti ti-plus"></i> Add Month</button>
              <button class="result-btn" onclick="fcReset()"><i class="ti ti-refresh"></i> Start Fresh</button>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
            ...
            <div class="form-group">
              <label class="form-label">Releases planned this year</label>
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                <input class="form-input" id="fc-releases" type="number" min="0" placeholder="e.g. 6"/>
                <select class="form-input" id="fc-reltype"><option value="songs">Songs</option><option value="albums">Albums</option></select>
              </div>
            </div>
          </div>
          <div class="form-group">
        ...
        <div class="gen-panel">
          ...
          <div class="form-group">
            ...
            <div style="border:1px dashed var(--border);border-radius:8px;padding:14px;text-align:center;cursor:pointer;background:#0f0f1a;" onclick="document.getElementById('lv-audio').click()">
              ...
              <input type="file" id="lv-audio" accept="audio/*,video/mp4,.mp3,.m4a,.wav,.aac,.ogg,.flac,.aif,.aiff" style="display:none;" onchange="lvLoadAudio(this)"/>
            </div>
          </div>
          <div class="form-group">
...
function switchTool(tool, sidebarBtn) {
  ...
  if(tool === 'forecast' && !document.querySelector('#fc-rows .fc-row')) {
    ...
    try {
      ...
      if (saved && saved.rows && saved.rows.length >= 2 && saved.ts && (Date.now() - saved.ts) < 2 * 60 * 60 * 1000) {
...
async function generateArt() {
  ...
  document.getElementById('art-loading-text').textContent = (document.getElementById('art-img-upload').files || []).length
  ...
  try {
    ...
    const fetchT = (url, body) => {
      const ctl = new AbortController();
      const timer = setTimeout(() => ctl.abort(), 75000);
      return fetch(url, { method: 'POST', headers: hdrs, body: JSON.stringify(body), signal: ctl.signal })
        .then(r => r.json())
        .finally(() => clearTimeout(timer));
    };
    const callA = () => fetchT('/api/generate-art', {prompt: basePrompt, imageDatas, inspirationDatas});
    const callB = () => fetchT('/api/generate-art-gemini', {prompt: basePrompt});
    ...
    if (Boolean(gotUrl(resA)) !== Boolean(gotUrl(resB)) && !isLimit(resA) && !isLimit(resB)) {
    ...
    const failText = r => r.status === 'fulfilled'
      ? ((r.value && (r.value.details || r.value.error)) || 'unknown response')
      : (String(r.reason).includes('abort') ? 'timed out after 75s' : String(r.reason));
    if (!versionAUrl || !versionBUrl) {
      const parts = [];
      if (!versionAUrl) parts.push('Version A: ' + failText(resA));
      if (!versionBUrl) parts.push('Version B: ' + failText(resB));
      console.error('COVER DEBUG →', parts.join(' | '));
      const dbg = document.getElementById('art-error');
      if (dbg) {
    ...
    if(versionAUrl && versionBUrl) {
      ...
      addRecent('🎨', title + ' — Version A', 'Cover Art', versionAUrl);
      saveAsset('Cover Art', title + ' — Version A', versionAUrl, {prompt: userPrompt, genre: genre, mood: mood, coverArtist: coverArtist, palette: palette});
      if (versionBUrl) {
        addRecent('🎨', title + ' — Version B', 'Cover Art', versionBUrl);
        saveAsset('Cover Art', title + ' — Version B', versionBUrl, {prompt: userPrompt, genre: genre, mood: mood, coverArtist: coverArtist, palette: palette});
      }
...
function fcCsvLine(line, delim) {
  // proper CSV split: respects "quoted, fields"
...
const FC_MONTHS = {jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'};

function fcMonthKey(raw, dayFirst) {
  const d = String(raw || '').replace(/"/g, '').trim();
  if (!d) return null;
  let m = d.match(/(\d{4})[-\/\.](\d{1,2})/);                    // 2024-08, 2024/8
  if (m) return m[1] + '-' + String(m[2]).padStart(2, '0');
  m = d.match(/(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})/);            // 8/1/2024 (US) or 01/08/2024 (intl)
  if (m) { const mo = dayFirst ? m[2] : m[1]; if (+mo >= 1 && +mo <= 12) return m[3] + '-' + String(mo).padStart(2, '0'); return null; }
  m = d.toLowerCase().match(/(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[\s\-\.,]+(\d{4})/); // Aug 2024
  if (m) return m[2] + '-' + FC_MONTHS[m[1]];
  m = d.toLowerCase().match(/(\d{4})[\s\-\.,]+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*/); // 2024 Aug
  if (m) return m[1] + '-' + FC_MONTHS[m[2]];
  return null;
}

let fcCsvReports = [];   // each uploaded report kept separately: {name, months:{'YYYY-MM':amt}} — removable
let fcCurrency = 'USD';  // detected from the money column header, e.g. "Net Royalties (AUD)"
function fcCur() { return { USD:'$', AUD:'A$', NZD:'NZ$', CAD:'C$', EUR:'\u20ac', GBP:'\u00a3' }[fcCurrency] || '$'; }
let fcFxCache = {};
const FC_FX_APPROX = { AUD: 0.66, NZD: 0.61, CAD: 0.73, EUR: 1.08, GBP: 1.27 }; // last-resort ballparks
async function fcFxRate(from) {
  if (fcFxCache[from]) return fcFxCache[from];
  // 1) our own backend (server-side lookup — immune to ad-blockers)
  try {
    const r = await fetch('/api/fx?from=' + from);
    const j = await r.json();
    if (j && j.rate > 0) { fcFxCache[from] = { rate: j.rate, approx: false }; return fcFxCache[from]; }
  } catch (e) { console.warn('FX via backend failed:', e); }
  // 2) direct provider from the browser
  try {
    const r = await fetch('https://api.frankfurter.app/latest?from=' + from + '&to=USD');
    const j = await r.json();
    if (j && j.rates && j.rates.USD) { fcFxCache[from] = { rate: j.rates.USD, approx: false }; return fcFxCache[from]; }
  } catch (e) { console.warn('FX direct failed:', e); }
  // 3) approximate baked rate — conversion still happens, clearly labeled
  if (FC_FX_APPROX[from]) return { rate: FC_FX_APPROX[from], approx: true };
...
function fcParseCsv(input) {
  ...
  let done = 0;
  const finish = (f, ok) => {
    if (!ok) document.getElementById('fc-parse-status').textContent =
      'Couldn\'t read "' + f.name + '" — send us the header row and we\'ll add that format. You can still type months below.';
    if (++done === files.length) fcCsvRender();
  };
  files.forEach(f => {
    ...
    reader.onload = e => {
      ...
      try {
        if (isExcel) {
          ...
          text = XLSX.utils.sheet_to_csv(wb.Sheets[wb.SheetNames[0]], { rawNumbers: true });
        } else {
          text = e.target.result;
        }
      } catch (err) { finish(f, false); return; }
      finish(f, fcIngestText(text, f.name));
    };
    reader.onerror = () => finish(f, false);
    if (isExcel) reader.readAsArrayBuffer(f); else reader.readAsText(f);
  });
}

function fcIngestText(text, fname) {
  try {
        const lines = text.split(/\r\n|\r|\n/).filter(l => l.trim());
        const first = lines[0];
        // delimiter: tab beats semicolon beats comma (DistroKid ships TSV)
        const counts = { '\t': (first.match(/\t/g)||[]).length, ';': (first.match(/;/g)||[]).length, ',': (first.match(/,/g)||[]).length };
        const delim = counts['\t'] >= 2 ? '\t' : (counts[';'] > counts[','] ? ';' : ',');
        // headerless DistroKid-style file: first line is DATA (col2 = YYYY-MM, col13 = float)
        const probe = fcCsvLine(first, delim);
        const dkSig = r => r.length >= 14 && /^\d{4}-\d{2}$/.test((r[2]||'').trim()) && !isNaN(parseFloat(r[13]));
        const dkHeaderless = lines.slice(0, 5).some(l => dkSig(fcCsvLine(l, delim)));
        if (dkHeaderless) {
          const months = {};
          let added = 0;
          lines.forEach(l => {
            const r = fcCsvLine(l, delim);
            const mk = (r[2]||'').trim();
            const amt = parseFloat(r[13]);
            if (/^\d{4}-\d{2}$/.test(mk) && !isNaN(amt)) { months[mk] = (months[mk]||0) + amt; added++; }
          });
          if (!added) throw new Error('no dated rows');
          fcCsvReports.push({ name: fname, months: months });
          return true;
        }
        const header = probe.map(h => h.toLowerCase().replace(/"/g, '').trim());
        // score columns by name: money wants earnings words, hates counting words
        let moneyIdx = -1, moneyBest = 0, dateIdx = -1, dateBest = 0;
        header.forEach((h, i) => {
          let ms = 0;
          if (/earn/.test(h)) ms += 5;
          if (/royal/.test(h)) ms += 5;
          if (/net|payout/.test(h)) ms += 4;
          if (/revenue/.test(h)) ms += 3;
          if (/amount|usd|total/.test(h)) ms += 2;
          if (/stream|unit|quantit|count|song|track|#|plays|downloads/.test(h)) ms -= 10;
          if (ms > moneyBest) { moneyBest = ms; moneyIdx = i; }
          let ds = 0;
          if (/period/.test(h)) ds += 5;
          if (/month/.test(h)) ds += 4;
          if (/statement|reporting/.test(h)) ds += 3;
          if (/date|sale/.test(h)) ds += 2;
          if (/store|country|name|title|artist|type/.test(h)) ds -= 10;
          if (ds > dateBest) { dateBest = ds; dateIdx = i; }
        });
        if (moneyIdx === -1) throw new Error('no money column');
        const curM = header[moneyIdx].match(/\b(aud|usd|eur|gbp|cad|nzd)\b/) || header.join(' ').match(/\b(aud|nzd|cad|eur|gbp)\b/);
        if (curM) fcCurrency = curM[1].toUpperCase();
        const rows = lines.slice(1).map(l => fcCsvLine(l, delim));
        // slash-date format: if any first number > 12 it's day-first (01/09/2025 = 1 Sep, not Jan 9)
        let dayFirst = false;
        if (dateIdx !== -1) {
          for (const r of rows) {
            const dm = String(r[dateIdx] || '').match(/(\d{1,2})[-\/](\d{1,2})[-\/]\d{4}/);
            if (dm) { if (+dm[1] > 12) { dayFirst = true; break; } if (+dm[2] > 12) { dayFirst = false; break; } }
          }
        }
        const months = {};
        let added = 0;
        rows.forEach(r => {
          const amt = parseFloat(String(r[moneyIdx] || '').replace(/[",$\s]/g, ''));
          if (isNaN(amt)) return;
          const key = dateIdx !== -1 ? fcMonthKey(r[dateIdx], dayFirst) : null;
          if (!key) return;
...
function fcCsvClean(monthly) {
  // trim pre-launch dead months + set aside an incomplete trailing month
  let lead = 0, partial = false;
  if (monthly.length >= 4) {
    const peak = Math.max.apply(null, monthly.map(m => m.amount));
    while (lead < monthly.length - 3 && monthly[lead].amount < 5 && monthly[lead].amount < peak * 0.01) lead++;
    const body = monthly.slice(lead);
    if (body.length >= 4) {
      const prev3 = body.slice(-4, -1).map(m => m.amount).sort((a, b) => a - b);
      const med = prev3[1];
      if (med > 0 && body[body.length - 1].amount < med * 0.4) partial = true;
    }
  }
  return { monthly: monthly.slice(lead, partial ? monthly.length - 1 : monthly.length), lead: lead, partial: partial };
}

function fcRemoveReport(i) {
  fcCsvReports.splice(i, 1);
  fcCsvRender();
}

function fcCsvRender() {
  const esc = t => String(t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;');
  const chips = document.getElementById('fc-reports');
  if (chips) chips.innerHTML = fcCsvReports.map((r, i) =>
    '<span style="display:inline-flex;align-items:center;gap:6px;background:rgba(0,255,209,0.08);border:1px solid rgba(0,255,209,0.35);border-radius:20px;padding:3px 10px;font-size:11px;color:#fff;">' +
    esc(r.name) + ' <span style="color:var(--muted);">· ' + Object.keys(r.months).length + ' mo</span>' +
    '<span onclick="fcRemoveReport(' + i + ')" title="Remove this report" style="cursor:pointer;color:var(--muted);font-weight:700;padding:0 2px;">✕</span></span>'
  ).join('');
  const stack = {};
  fcCsvReports.forEach(r => Object.keys(r.months).forEach(k => { stack[k] = (stack[k] || 0) + r.months[k]; }));
  const keys = Object.keys(stack).sort();
  if (!keys.length) {
    document.getElementById('fc-label').textContent = 'Click to upload your earnings CSV';
    document.getElementById('fc-rows').innerHTML = '';
    fcAddRow(); fcAddRow(); fcAddRow();
    document.getElementById('fc-parse-status').textContent = '';
    fcMonthly = null;
    return;
  }
  const cleaned = fcCsvClean(keys.map(k => ({ month: k, amount: Math.round(stack[k] * 100) / 100 })));
  fcMonthly = cleaned.monthly;
  let cleanNote = fcCurrency !== 'USD' ? ' · amounts are in ' + fcCurrency : '';
  if (cleaned.lead) cleanNote += ' · trimmed ' + cleaned.lead + ' near-zero month' + (cleaned.lead > 1 ? 's' : '') + ' from before your catalog was live';
  if (cleaned.partial) cleanNote += ' · your latest month looks incomplete (statements lag), so it\'s set aside — type it back in below if it\'s real';
  document.getElementById('fc-rows').innerHTML = '';
  fcMonthly.slice(-24).forEach(m => fcAddRow(m.month, m.amount));
  const total = fcMonthly.reduce((a, b) => a + b.amount, 0);
  const nRep = fcCsvReports.length;
  document.getElementById('fc-label').textContent = '✓ ' + nRep + ' report' + (nRep > 1 ? 's' : '') + ' loaded — add more anytime';
  if (keys.length === 1) {
    document.getElementById('fc-parse-status').innerHTML =
      '✓ <strong style="color:var(--neon);">1 month found</strong> ($' + total.toFixed(2) + '). Forecasts need 2+ — <strong>upload your other monthly reports</strong> (they stack automatically) or add months by hand below.' + cleanNote;
  } else {
    document.getElementById('fc-parse-status').innerHTML =
      '✓ <strong style="color:var(--neon);">' + fcMonthly.length + ' months ready</strong> (' + keys.length + ' found in ' + nRep + ' report' + (nRep > 1 ? 's' : '') + ') — forecast will use ' + fcCur() + total.toFixed(2) + '. Check the rows, add more, or hit Forecast.' + cleanNote;
  }
}

function fcReset() {
  try { localStorage.removeItem('fcSaved'); } catch(e) {}
  fcCsvReports = [];
  const chips = document.getElementById('fc-reports'); if (chips) chips.innerHTML = '';
  document.getElementById('fc-label').textContent = 'Click to upload your earnings CSV';
  document.getElementById('fc-rows').innerHTML = '';
  fcAddRow(); fcAddRow(); fcAddRow();
  document.getElementById('fc-genre').value = '';
  document.getElementById('fc-city').value = '';
  document.getElementById('fc-releases').value = '';
  const rt = document.getElementById('fc-reltype'); if (rt) rt.value = 'songs';
  fcCurrency = 'USD';
  document.getElementById('fc-result').style.display = 'none';
  document.getElementById('fc-placeholder').style.display = 'block';
  document.getElementById('fc-parse-status').textContent = '';
  fcMonthly = null;
}
...
function fcSongsPlanned() {
  const n = parseFloat(document.getElementById('fc-releases').value) || 0;
  const t = (document.getElementById('fc-reltype') || {}).value || 'songs';
  return t === 'albums' ? n * 8 : n;   // an album ~8 songs of release energy
}
...
function fcProject(monthly) {
  ...
  function run(g0, viral) {
    // momentum fades: month-i growth = g0 * 0.965^i, so curves bend toward a
    // plateau instead of compounding forever (or bleeding to zero on a decline)
    let m = avgRecent, total = 0;
    const marks = { series: [] };
    for (let i = 1; i <= 60; i++) {
      m = m * (1 + g0 * Math.pow(0.965, i));
      let monthVal = m;
  ...
  const steadyG = Math.max(-0.03, Math.min(0.02, trend));       // coast: mild version of current trend, fading to long-tail
  // your release plan bends the growth curve: ~6 songs/yr is the baseline plan;
  // more output compounds harder, a thin plan grows slower (capped either way)
  const relAdj = Math.max(-0.01, Math.min(0.02, (fcSongsPlanned() - 6) * 0.002));
  const growthBase = Math.min(0.08, Math.max(0.03, trend + 0.06));
  const growthG = Math.min(0.09, Math.max(0.02, growthBase + relAdj)); // working the plan: release cadence bends it, momentum fades
  return {
...
function fcMoney(v) {
  v = v || 0;
  return v >= 1000 ? fcCur() + (v/1000).toFixed(v >= 100000 ? 0 : 1) + 'k' : fcCur() + Math.round(v);
}
...
function fcExportImage() {
  if (!_fcLast) return;
  const { stats, scenarios, data } = _fcLast;
  const money = v => fcCur() + (v || 0).toLocaleString();
  const W = 1080, H = 1500;
  const c = document.createElement('canvas');
  ...
  // background
  const bg = ctx.createLinearGradient(0, 0, 0, H);
  bg.addColorStop(0, '#0a0a12'); bg.addColorStop(1, '#05050a');
  ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
  // header
  ctx.fillStyle = '#00FFD1'; ctx.font = '800 26px Arial'; ctx.textAlign = 'left';
  ctx.fillText('STUDIOSOUND.AI', 60, 80);
  ctx.fillStyle = '#ffffff'; ctx.font = '900 54px Arial';
  ctx.fillText('EARNINGS FORECAST', 60, 150);
  ctx.fillStyle = '#7a7a92'; ctx.font = '600 24px Arial';
  ctx.fillText(new Date().toLocaleDateString() + '  ·  5-year projection', 60, 190);
  // numbers strip
  ctx.fillStyle = '#e8e8f5'; ctx.font = '700 30px Arial';
  ctx.fillText('Monthly average: ' + money(stats.avgMonthly) + '    Trend: ' + (stats.trend > 0 ? '+' : '') + stats.trend + '%/mo', 60, 260);
  // chart snapshot
  const chart = document.getElementById('fc-chart');
  if (chart) ctx.drawImage(chart, 60, 300, W - 120, (W - 120) * (chart.height / chart.width));
  let y = 300 + (chart ? (W - 120) * (chart.height / chart.width) : 0) + 80;
  // scenario rows
  const rows = [
    ['IF YOU COAST', scenarios.steady, '#9a9ab2'],
    ['IF YOU WORK THE PLAN', scenarios.growth, '#00FFD1'],
    ['PLAN + ONE VIRAL HIT', scenarios.breakout, '#FFD700']
  ];
  rows.forEach(([label, sc, color]) => {
    ctx.fillStyle = color; ctx.font = '800 28px Arial';
    ctx.fillText(label, 60, y);
    ctx.fillStyle = '#ffffff'; ctx.font = '600 28px Arial';
    ctx.fillText('1yr ' + money(sc.y1) + '    3yr ' + money(sc.y3) + '    5yr ' + money(sc.y5), 460, y);
    y += 56;
  });
  y += 20;
  // the gap line
  const gap = (scenarios.growth.y5 || 0) - (scenarios.steady.y5 || 0);
  ctx.fillStyle = '#00FFD1'; ctx.font = '800 32px Arial';
  ctx.fillText('The work is worth ' + money(gap) + ' over 5 years.', 60, y);
  y += 70;
  // moves
  if (data.moves && data.moves.length) {
    ctx.fillStyle = '#7a7a92'; ctx.font = '800 22px Arial';
    ctx.fillText('MOVES THAT CHANGE YOUR CURVE', 60, y); y += 44;
    ctx.fillStyle = '#e8e8f5'; ctx.font = '500 24px Arial';
    data.moves.slice(0, 4).forEach(m => {
      const words = ('⚡ ' + m).split(' ');
      let line = '';
      words.forEach(wd => {
        if (ctx.measureText(line + wd).width > W - 140) { ctx.fillText(line, 60, y); y += 36; line = ''; }
        line += wd + ' ';
      });
  ...
  // footer
  ctx.fillStyle = '#5a5a72'; ctx.font = '500 20px Arial';
  ctx.fillText('Scenario estimates — not guarantees, not financial advice.', 60, H - 70);
  ctx.fillStyle = '#00FFD1'; ctx.font = '700 22px Arial';
  ctx.fillText('studiosound.ai — Create. Release. Elevate.', 60, H - 36);
  // download
  const a = document.createElement('a');
  a.download = 'earnings-forecast.png';
...
function fcExportPdf() {
  ...
  w.document.write('<html><head><title>Earnings Forecast — StudioSound.ai</title><style>' +
    ...
    '<h2>Your Numbers</h2><p>Recent monthly average: <strong>' + money(stats.avgMonthly) + '</strong> · Trend: <strong>' + (stats.trend > 0 ? '+' : '') + stats.trend + '%/mo</strong> · Based on ' + stats.monthsOfData + ' months of data' + (stats.fx ? ' \u00b7 ' + stats.fx : '') + '</p>' +
    ...
    (data.scenario_notes ? '<p><strong>Steady:</strong> ' + (data.scenario_notes.steady||'') + '</p><p><strong>Growth:</strong> ' + (data.scenario_notes.growth||'') + '</p><p><strong>Breakout:</strong> ' + (data.scenario_notes.breakout||'') + '</p>' : '') +
    (function(){
      if (!scenarios.growth.series) return '';
      var rows = '';
      var prevS = 0, prevG = 0;
      var mn = ['Month 1','Month 2','Month 3','Month 4','Month 5','Month 6','Month 7','Month 8','Month 9','Month 10','Month 11','Month 12'];
      for (var i = 0; i < 12; i++) {
        var sM = scenarios.steady.series[i] - prevS; prevS = scenarios.steady.series[i];
        var gM = scenarios.growth.series[i] - prevG; prevG = scenarios.growth.series[i];
        rows += '<tr><td>' + mn[i] + '</td><td>' + money(Math.round(sM)) + '</td><td>' + money(Math.round(gM)) + '</td></tr>';
      }
      return '<h2>Your First Year, Month by Month</h2><table><tr><th>Month</th><th>Steady path</th><th>Growth path</th></tr>' + rows + '</table>';
    })() +
    ...
    (data.milestones && data.milestones.length ? '<h2>Milestones Ahead</h2>' + data.milestones.map(m => '<div class="move">• ' + m + '</div>').join('') : '') +
    (data.watch_outs && data.watch_outs.length ? '<h2>Watch Out For</h2>' + data.watch_outs.map(m => '<div class="move">• ' + m + '</div>').join('') : '') +
...
function fcRenderFull(stats, scenarios, data) {
  ...
  html += `<div class="plan-item"><div class="plan-date">YOUR NUMBERS</div><div class="plan-detail">Recent monthly average: <strong style="color:#fff;">${money(stats.avgMonthly)}</strong> · Trend: <strong style="color:#fff;">${stats.trend > 0 ? '+' : ''}${stats.trend}%/mo</strong> · Based on ${stats.monthsOfData} months of data${stats.fx ? ' \u00b7 ' + stats.fx : ''}</div></div>`;
  ...
  if (data.milestones && data.milestones.length) {
    html += `<div class="plan-item"><div class="plan-date">MILESTONES AHEAD</div>` +
      data.milestones.map(m => `<div class="plan-detail" style="margin-bottom:4px;">🏁 ${m}</div>`).join('') + `</div>`;
  }
  if (data.watch_outs && data.watch_outs.length) {
    html += `<div class="plan-item"><div class="plan-date">WATCH OUT FOR</div>` +
      data.watch_outs.map(m => `<div class="plan-detail" style="margin-bottom:4px;">⚠️ ${m}</div>`).join('') + `</div>`;
  }
  ...
  if (data.analysis) {
    html += `<div class="result-actions" style="margin-top:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px;">
      <button class="result-btn primary" onclick="fcExportImage()" style="justify-content:center;"><i class="ti ti-photo-down"></i> Save as Image</button>
      <button class="result-btn" onclick="fcExportPdf()" style="justify-content:center;"><i class="ti ti-file-download"></i> PDF (desktop)</button>
    </div>`;
  }
...
async function runForecast() {
  ...
  if (!monthly || monthly.length < 2) { alert('Almost there — one more month and we can read your trend. Upload another monthly report (they stack automatically) or add a month by hand.'); return; }
  ...
  try {
    localStorage.setItem('fcSaved', JSON.stringify({
      ...
      releases: document.getElementById('fc-releases').value + ' ' + ((document.getElementById('fc-reltype')||{}).value || 'songs'),
      currency: fcCurrency
    }));
  } catch(e) {}

  // Statements in AUD/EUR/etc: convert to USD at today's rate so every forecast speaks one currency.
  // Rows keep the statement's own currency (they should match what the distributor shows).
  let fxNote = '';
  if (fcCurrency !== 'USD') {
    const fromCur = fcCurrency;
    try {
      const fx = await fcFxRate(fromCur);
      monthly.forEach(m => { m.amount = Math.round(m.amount * fx.rate * 100) / 100; });
      fxNote = 'Converted from ' + fromCur + ' to USD at ' + fx.rate.toFixed(4) + (fx.approx ? ' (approximate rate)' : ' (today\'s rate)');
      fcCurrency = 'USD';
    } catch(e) {
      fxNote = 'Shown in ' + fromCur + ' (USD rate unavailable right now)';
    }
  }
  ...
  try {
    const res = await fetch('/api/generate-forecast', {
      ...
      body: JSON.stringify({
        ...
        releasesPerYear: (document.getElementById('fc-releases').value.trim() + ' ' + (((document.getElementById('fc-reltype')||{}).value) || 'songs')).trim(),
        currency: fcCurrency
      })
    });
    const data = await res.json();
...
async function lvRun(record) {
  ...
  if(record) {
    ...
    // live countdown so nobody thinks it's stuck — and try to keep the phone screen awake
    let lvWake = null;
    try { if (navigator.wakeLock) navigator.wakeLock.request('screen').then(w => lvWake = w).catch(()=>{}); } catch(e) {}
    const lvExpEnd = Date.now() + durForStatus * 1000;
    const lvExpTimer = setInterval(() => {
      const left = Math.max(0, Math.ceil((lvExpEnd - Date.now()) / 1000));
      const st = document.getElementById('lv-export-status');
      if (left > 0) {
        st.textContent = '🎬 Rendering your video in real time — ' + Math.floor(left/60) + ':' + String(left%60).padStart(2,'0') + ' left. Keep this screen open (it\'s filming the video as it plays).';
      } else {
        st.textContent = 'Finishing up — your download starts in a moment...';
        clearInterval(lvExpTimer);
      }
      if (window._lvExpDone) { clearInterval(lvExpTimer); if (lvWake) { try { lvWake.release(); } catch(e){} } }
    }, 1000);
    window._lvExpDone = false;
    document.getElementById('lv-export-status').textContent = '🎬 Rendering your video in real time (' + durForStatus.toFixed(0) + 's) — keep this screen open...';
  }
